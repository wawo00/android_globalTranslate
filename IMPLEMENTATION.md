...
- `canRetrieveWindowContent` - 允许获取窗口内容
- `flagReportViewIds` - 报告视图ID
- `flagRetrieveInteractiveWindows` - 获取交互式窗口
...
2. ✅ 可以获取文本内容、控件类型等信息
3. ✅ 可以模拟点击、滚动等用户操作
4. ✅ 可以修改可编辑控件（EditText）的内容
5. ❌ 无法修改不可编辑控件的视图属性
6. ❌ 无法修改其他应用的 View 样式

**为什么无法修改颜色？**

AccessibilityNodeInfo 是 View 的快照，不是实际的 View 对象：
- 无法访问 View 的 `setTextColor()` 方法
- 无法修改 View 的属性
- 只能使用无障碍 API 提供的有限操作

**可行的替代方案:**
- 在自己的应用中实现类似功能（完全控制）
- 使用 Xposed 框架（需要 Root）
- 开发系统级应用（需要系统签名）
